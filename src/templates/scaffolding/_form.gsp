...
<div class="row">
    <section class="col col-6">
        <label class="label"><g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" /> <% if (required) { %><span class="required-indicator">*</span><% } %></label>
        <label class="input \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'state-error')} ${required ? 'required' : ''}"> <i class="icon-append fa fa-question-circle"></i>
            ${renderEditor(p)}
            <b class="tooltip tooltip-top-right">
                <i class="fa fa-warning txt-color-teal"></i> 
                <g:message code="${domainClass.propertyName}.${prefix}${p.name}.tooltip" default="${p.naturalName} tooltip" /></b> 
        </label>
    </section> 
</div>
<%  } %>
