package com.lucasian.nsjp

import org.springframework.web.servlet.support.RequestContextUtils as RCU

class CustomTagLib {
    static namespace = "luc"
    /*
    ...
    */
    def paginate = { attrs ->
        ...
        writer<<'<div class="dt-row dt-bottom-row">'
        writer<<'<div class="row">'
        writer<<'<div class="col-sm-6"><div class="dataTables_info" id="datatable_col_reorder_info">'
        writer<<'Mostrando '
        if (currentstep > firststep) {
            writer<<(currentstep-1)*max
            if((currentstep*max)>total){
                writer<<' al '+total
            }else{
                writer<<' al '+(currentstep)*max
            }
        }else if (steps && laststep > firststep) {            
            writer<<firststep+' al '+currentstep*max
        }else{
            writer<<'Mostrando '+firststep+' al '+total
        }    
        writer<<' de '+total+' registros'
        writer<<'</div>'
        writer<<'</div>'
        writer<<'<div class="col-sm-6 text-right">'
        writer<<'<div class="dataTables_paginate paging_bootstrap">'        
